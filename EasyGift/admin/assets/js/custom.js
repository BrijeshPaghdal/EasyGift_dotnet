$(document).ready(function() {
  ...
  if (url.split('?').includes('product-list.php') && window.location.search.includes('shop_id')) {
  ...
  else if (url.split('?').includes('product-list.php'))
  {
    ajaxProductList();
  ...
  function productStatus(id, s_id=0) {
    var status = $(id).val();
    var status = status.split('/');
  ...
  function ajaxOrderList(shop_id=0) {
    $.ajax({
