...
namespace EasyGift_API.Controllers
{
    public class CustomerController : GenericController<Customer, CustomerDTO, CreateCustomerDTO, UpdateCustomerDTO>
    {
        ...
        [HttpGet("{id:int}/GetOrderCompleteNotification")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<APIResponse>> GetOrderCompleteNotification(int id)
        {
            try
            {

                dynamic datas = await _db.GetOrderCompleteNotification(id);
